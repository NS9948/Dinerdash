...
const TableCard = ({id,name,status,initials,seats}) => {
  ...
  const handleClick = (name) => {
    ...
    const table = {tableId: id, tableName: name}
  ...
  return (
    <div onClick={() => handleClick(name)} key ={id} className='w-[300px] hover:bg-[#1f1f1f] bg-[#262626] p-4 rounded-lg mb-4 cursor-pointer'>
        <div className='flex items-center justify-between px-1'>
            <h1 className='text-[#f5f5f5] text-xl font-semibold'>Table<FaLongArrowAltRight className='text-[#ababab] ml-2 inline-flex'/>{name}</h1>
